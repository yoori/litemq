...
namespace StringManip
{
  ...
  {
    return (hex_to_int(major) << 4) | hex_to_int(minor);
  }

  uint32_t
  crc32(uint32_t crc, const void* data, size_t size)
    throw ()
  {
  ...
  {
    static const char* FUN = "StringManip::hex_decode()";
    ...
    dst.resize(size);
    char* data = reinterpret_cast<char*>(dst.data());
